#ifndef __CHAMELEON_HASH_H_
#define __CHAMELEON_HASH_H_
...
void chameleon_hash_init_sysparams(const unsigned int security_bits,
				   struct ch_sys_params *);
void chameleon_hash_init_message(const struct ch_sys_params *,
				 struct message *msg);
void chameleon_hash_init_randomness(const struct ch_sys_params *,
				    struct randomness *rnd);
void chameleon_hash_init_digest(const struct ch_sys_params *,
				struct digest *dgt);
...
void chameleon_hash_collision(const struct ch_sys_params *sys,
			      ...
			      const struct randomness *rnd1,
			      const struct message *msg2,
			      struct randomness *rnd2);
